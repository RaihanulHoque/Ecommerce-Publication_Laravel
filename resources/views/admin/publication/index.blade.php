...
@section('title')
    Admin Panel | Manage Publications
@endsection
                ...
                <div class="col-Select inputs12 data_tables">
                    <div class="card ">
                        <div class="card-header bg-white">
                            <i class="fa fa-table"></i> Manage Publications
                        </div>
                        <div class="card-body ">

                            <a class="btn btn-success adv_cust_mod_btn m-b-20" data-toggle="modal"
                               data-href="#responsive" href="#responsive">Add New Publication</a>
                            <table id="example2" class="display table table-stripped table-bordered">
                                ...
                                @foreach($data as $row)
                                    <tr>
                                        <td>{{$i++}}</td>
                                        <td><img src="{{ URL::to('/') }}/uploads/publications/{{ $row->image }}" class="img-thumbnail" width="75" /></td>
                                        <td>{{ $row->title }}</td>
                                        ...
                                        <td>
                                            <a href="{{route('publications.show', $row->id)}}" data-toggle="tooltip" data-placement="top" title="View User"><i class="fa fa-eye text-success"></i></a>&nbsp; &nbsp;
                                            <a href="{{route('publications.edit', $row->id)}}" class="edit" data-toggle="tooltip" data-placement="top" title="Edit" ><i class="fa fa-pencil text-warning"></i></a>&nbsp; &nbsp;
                                            ...
                                            <form id="delPublicationForm{{$row->id}}" action="{{route('del-publication')}}" method="POST">
                                                @csrf
                                ...
                                <script>
                                    $('.deleteBtn').click(function () {
                                        ...
                                        var check = confirm('Are you sure to delete this Publication??');
                                        if(check){
                                            document.getElementById('delPublicationForm'+publicationID).submit();
                                        }
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header bg-success">
                    <h4 class="modal-title text-white">Add New Publication</h4>
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                </div>
                <div class="modal-body">
                    <div class="row">
                        <div class="col-md-12">
                            <form  id="categoryForm" method="post" action="{{ route('publications.store') }}" enctype="multipart/form-data" name="categoryForm" class="form-horizontal">
